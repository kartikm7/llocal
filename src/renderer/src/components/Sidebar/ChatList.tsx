...
export const ChatList = ({ className, ...props }: ComponentProps<'div'>): React.ReactElement => {
  ...
  return (
    <div className={twMerge('flex flex-col gap-2 overflow-auto', className)} {...props}>
      {chatList.map((val) => {
        return <Card key={val.date} onClick={() => handleClick(val.date)} className={`group relative cursor-pointer py-3 ${selectedChatIndex === val.date ? "opacity-100" : "opacity-50"} ${stream && "cursor-default"} hover:opacity-100 transition-opacity`}>
